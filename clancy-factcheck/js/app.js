...
(function () {
  ...
  /* ---------- state ---------- */
  const state = { tab: "overview", q: "", cat: "all", checks: {}, opened: {} };
  const TABS = [
    ["overview", "Overview"], ["timeline", "Jan. 24 timeline"], ["myths", "Misinformation"],
    ["claims", "Claims about Patrick"], ["evidence", "Why he's ruled out"], ["criticism", "Fair criticism"],
  ...
  function tabMatches() {
    ...
    return {
      timeline: D.TIMELINE.filter((r) => matches(r.t, r.x)).length,
  ...
  function secnav() {
    const order = ["overview", "timeline", "myths", "claims", "evidence", "criticism", "sources", "about"];
  ...
  /* ---------- views ---------- */
  function vOverview() {
    const allText = [...D.STATUS, ...D.TIMELINE.map((t) => t.x), ...D.MYTHS.map((m) => m.claim + m.fact),
      ...D.CLAIMS.map((c) => c.claim + c.rec), ...D.EVIDENCE.map((e) => e.x), ...D.CRITICISMS, ...D.CHECKS].join(" ");
    const linkCount = (allText.match(linkRe) || []).length;
    return `
    <p class="lede">Patrick Clancy was on camera fifteen to twenty minutes away. Lindsay Clancy does not deny the killings. Everything else people are "just asking" about is checked here, and every underlined phrase opens the report it came from.</p>
    <p class="meta">${D.MYTHS.length} pieces of misinformation, ${D.CLAIMS.length} theories about Patrick, ${D.EVIDENCE.length} lines of evidence, ${linkCount} linked facts, ${Object.keys(S).length} sources. Updated ${D.UPDATED}.</p>
    ...
    <div class="list">
      <button class="row" data-tab="timeline">Jan. 24 timeline<span class="sub">Minute by minute, with who was where</span></button>
      ...
      <button class="row" data-tab="claims">Claims about Patrick<span class="sub">${D.CLAIMS.length} theories from TikTok, Reddit and YouTube, each against the record</span></button>
  ...
  function vClaims() {
    const cats = [["all", "All"], ["A", "Timeline and alibi"], ["B", "Scene and forensics"], ["C", "Digital evidence"], ["D", "Behavior and demeanor"]];
  ...
  function vSources() {
    const use = {};
    const all = [...D.STATUS, ...D.TIMELINE.map((t) => t.x), ...D.MYTHS.map((m) => m.claim + " " + m.fact),
      ...D.CLAIMS.map((c) => c.claim + " " + c.rec), ...D.EVIDENCE.map((e) => e.x), ...D.CRITICISMS, ...D.CHECKS].join(" ");
  ...
  const VIEWS = { overview: vOverview, timeline: vTimeline, myths: vMyths, claims: vClaims, evidence: vEvidence, criticism: vCriticism, sources: vSources, about: vAbout };
  ...
  function render() {
    ...
    $("#tabs").innerHTML = TABS.map(([k, l]) => {
      ...
      const n = searching ? (tm[k] === undefined ? "" : tm[k]) : base;
    ...
    $$("#dock button[data-more]").forEach((b) => b.classList.toggle("on", ["evidence", "criticism", "sources", "about"].includes(state.tab)));
  ...
  document.addEventListener("click", (e) => {
    const t = e.target.closest("[data-tab],[data-cat],[data-expandall],[data-toggle],[data-share],[data-copy],[data-more],[data-pref] button");
    if (!t) return;
